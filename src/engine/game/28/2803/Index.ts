 ...
 */
export class Game2803 extends CtrlGame {
  ...
  refreshBetConfig() {
    if (this.isCan()) {
      this.SendData(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_UPDATE_CONFIG);
      return true;
    }
    return false;
  }
  ...
  sceneChange() {
    if (this.isCan()) {
      this.SendData(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_SCENE_CHANGE);
      return true;
    }
    return false;
  }
  ...
  getHistoryRecord() {
    if (this.isCan()) {
      this.SendData(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_HISTORY);
      return true;
    }
    return false;
  }
  ...
  getDynamicMultiple() {
    if (this.isCan()) {
      this.SendData(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_DYNAMIC_MULTIPLE);
      return true;
    }
    return false;
  }
  ...
  /**
   * @description: 用户下注
   * @param {CMD_2803} betScore
   * @return {*}
   */
  userBetScore(betScore: common.CMD_C_PlaceBet) {
    if (this.isCan() && betScore) {
      this.SendData(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_C_PLACE_JETTON, betScore);
      return true;
    }
    return false;
  }
  ...
  initEvent(): void {
    ...
    // 配置刷新
    this.RegistMessageEx(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_UPDATE_CONFIG, this.onRefreshBaseConfig.bind(this), CMD_2803.tagBaseConfig);
    // 推送游戏场景变化
    this.RegistMessageEx(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_SCENE_CHANGE, this.onSceneChange.bind(this), CMD_2803.CMD_S_SceneChange);
    // 进入房间后，推送最近50期的游戏记录。后续仅推送最新一期的开奖结果，不再推送全部记录。
    this.RegistMessageList(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_HISTORY, this.onHistoryRecord.bind(this), CMD_2803.tagRecordInfo);
    // 游戏结束
    this.RegistMessageEx(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_GAME_END, this.onGameEnd.bind(this), CMD_2803.CMD_S_GameEnd);
    // 获取动态赔率
    this.RegistMessageEx(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_DYNAMIC_MULTIPLE, this.onDynamicMultiple.bind(this), CMD_2803.CMD_S_DynamicMultiple);
    // 动态赔率下发完成
    this.RegistMessage(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_DYNAMIC_MULTIPLE_FINISH, this.onDynamicMultipleFinish.bind(this));
    // 监听用户下注信息
    this.RegistMessage(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_PLACE_JETTON, this.onUserBetInfo.bind(this));
    // 监听用户下注成功
    this.RegistMessage(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_PLACE_JETTON_SUCCESS, this.onUserBetSuccess.bind(this));
    // 监听用户下注失败
    this.RegistMessage(CMD_Game.MDM_GF_GAME, CMD_2803.SUB_S_PLACE_JETTON_FAILURE, this.onUserBetFaild.bind(this));
  }
  ...
  onGameSceneConfig(data: DataBuffer): void {
    let gameScene = data.toStruct(CMD_2803.CMD_S_GameScene);
   ...
   * @return {*}
   */
  private onRefreshBaseConfig(data: CMD_2803.tagBaseConfig) {
   ...
   * @return {*}
   */
  private onSceneChange(data: CMD_2803.CMD_S_SceneChange) {
   ...
   * @return {*}
   */
  private onHistoryRecord(list: Array<CMD_2803.tagRecordInfo>) {
   ...
   * @return {*}
   */
  private onGameEnd(data: CMD_2803.CMD_S_GameEnd) {
   ...
   * @return {*}
   */
  private onDynamicMultiple(data: CMD_2803.CMD_S_DynamicMultiple) {
  ...
  private onUserBetInfo(data: DataBuffer) {
    const placeBetInfo = new CMD_2803.CMD_S_PlaceBet();
    placeBetInfo.tagSpecialMultiple = [];
    placeBetInfo.tagCommonBetInfo = [];
    const placeBetHead = data.toStruct(CMD_2803.CMD_S_PlaceBetHead);

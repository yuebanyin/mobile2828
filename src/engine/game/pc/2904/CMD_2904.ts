...
export namespace CMD_2904 {
  ...
  // 抛出子类型的名称
  export const emBetSubTypeNames = ['emBetTypeRank', 'emBetTypeSum'];
  ...
  export enum emMultipleType {
    ...
    MT_RANK9_8, //第九名-8
    MT_RANK9_9, //第九名-9
    MT_RANK9_10 = 89, //第九名-10
    MT_RANK10_1 = 90, //第十名-1
    MT_RANK10_2, //第十名-2
    MT_RANK10_3, //第十名-3
    MT_RANK10_4, //第十名-4
    MT_RANK10_5, //第十名-5
    ...
    MT_SUM_17, //冠亚军和-17
    MT_SUM_18, //冠亚军和-18
    MT_SUM_19 = 122, //冠亚军和-19
    MT_SUM_DA = 123, //冠亚军和-大
    MT_SUM_XIAO, //冠亚军和-小
    MT_SUM_DAN, //冠亚军和-单
  ...
  export enum emResultPosType {
    ...
    RPT_R1_DS = 10, //冠军-单双
    //...
    RPT_R10_DS = 19, //第十名-单双
    RPT_R1_LH = 20, //冠军-龙虎
    //...
    RPT_R5_LH = 24, //第五名-龙虎
    RPT_SUM_DX, //冠亚军和-大小
    RPT_SUM_DS, //冠亚军和-单双
    RPT_COUNT = 27, //结果位置类型数目
  ...
  export const SOURCE_DATA_COUNT: number = 10; //源数据数量
  ...
  const mapMainType = new Map<number, string>([
    [emBetMainType.BTM_RANK_1, 'NUMBER_1'],
    [emBetMainType.BTM_RANK_2, 'NUMBER_2'],
    [emBetMainType.BTM_RANK_3, 'NUMBER_3'],
    [emBetMainType.BTM_RANK_4, 'NUMBER_4'],
    [emBetMainType.BTM_RANK_5, 'NUMBER_5'],
    [emBetMainType.BTM_RANK_6, 'NUMBER_6'],
    [emBetMainType.BTM_RANK_7, 'NUMBER_7'],
    [emBetMainType.BTM_RANK_8, 'NUMBER_8'],
    [emBetMainType.BTM_RANK_9, 'NUMBER_9'],
    [emBetMainType.BTM_RANK_10, 'NUMBER_10'],
    [emBetMainType.BTM_SUM, 'NUMBER_1_2_HE'],
  ]);

  // 下注子类型 - 冠军-第十名
  const mapSubRankType = new Map<number, string>([
    [emBetTypeRank.SBTR_DA, 'DA'],
    [emBetTypeRank.SBTR_XIAO, 'XIAO'],
    [emBetTypeRank.SBTR_DAN, 'DAN'],
    [emBetTypeRank.SBTR_SHUANG, 'SHUANG'],
    [emBetTypeRank.SBTR_LONG, 'LONG'],
    [emBetTypeRank.SBTR_HU, 'HU'],
  ]);

  // 下注子类型 - 冠亚军和
  const mapSubSumType = new Map<number, string>([
    [emBetTypeSum.SBTS_DA, 'DA'],
    [emBetTypeSum.SBTS_XIAO, 'XIAO'],
    [emBetTypeSum.SBTS_DAN, 'DAN'],
    [emBetTypeSum.SBTS_SHUANG, 'SHUANG'],
  ]);
  ...
  /**
   * @description: 获取子下注类型描述---如果是自选数字的情况下，根据主区域-加数字即可 --获取第一到第十名的子区域类型
   * @return {*}
   */
  export function GetGameBetSubRankType(sType: number): string {
    if (mapSubRankType.has(sType)) return mapSubRankType.get(sType);
    return '';
  }

  /**
   * @description: 获取子下注类型描述---如果是自选数字的情况下，根据主区域-加数字即可 --冠亚军和
   * @return {*}
   */
  export function GetGameBetSubSumType(sType: number): string {
    if (mapSubSumType.has(sType)) return mapSubSumType.get(sType);
    return '';
  }

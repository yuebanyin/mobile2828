 ...
 * @returns
 */
function SlovakLuckyFive() {
  const { formatMsg } = useLanguage();
  const { token, gameId } = useUserInfoStore();
  const [gameHandle, setGameHandle] = useState<Game3202>();
  const [gameScene, setGameScene] = useState(
    CreateObject(CMD_3202.CMD_S_GameScene)
  );
  const refContent = useRef(null);
  const refBetInputRef = useRef(null);
  const [gameMultiple, setGameMultiple] = useState(
    CreateArray(DWORD, [CMD_3202.emMultipleType.MT_COUNT])
  );
  ...
  const { fieldInfo } = getGameInfo(gameList, 3202);
    ...
    (msg: NetData, wsG: CtrlGame) => {
      switch (msg.dateType) {
        ...
        case GameRoute.gameScenConfig:
          {
            ...
            const scene = msg.data as CMD_3202.CMD_S_GameScene;
            console.log(
        ...
        case GameRoute.sceneChange:
          ...
          const scene1 = msg.data as CMD_3202.CMD_S_GameScene;
        ...
        case GameRoute.dynamicMultiple:
          ...
          setDyMultiple((r) => [
            ...(msg.data as CMD_3202.CMD_S_DynamicMultiple).stMultipleList,
            ...r,
          ]);
          break;
        ...
        case GameRoute.gameEnd:
          ...
          // eslint-disable-next-line no-case-declarations
          const gameEnd = msg.data as CMD_3202.CMD_S_GameEnd;
          setCurPeriodNumber(
  ...
  useEffect(() => {
    if (url && webkitId && !wsCC?.current) {
      ...
      wsCC.current = new Game3202(url, webkitId);
...
export default observer(SlovakLuckyFive);

...
import {
 ButtonDataType, ContentProps, ItemLayoutProps, zhButton 
} from '../rules';
...
const ItemLayout = forwardRef<RadioGroupRef, ItemLayoutProps>((props, ref) => {
 ...
 name, oddsKey, mainType, subType, number 
      ...
      if (children.length > 10 && subType === CMD_3202.emSubBetTypeDingWei.SBTD_NUM) {
        // 数字球的可选项目超过10，并且子下注类目是SBTD_NUM
        return (
      ...
      return (
        <div className='w-full h-102 d-f ai-c jc-c'>
          <span className='w-140 m-r-30 ta-c'>{name}</span>
          <span className='w-140 color-red'>{odds}</span>
        </div>
      );
    },
  ...
  return (
    <>
      <div className='w-full p-20 bg-gray p-r d-f fd-r ai-c'>
        ...
        <div className='p-a right-28 top-5' onClick={() => setShow(!show)}>
 ...
 * 斯洛伐克幸运5-整合
 */
export const ZhPage = forwardRef<RadioGroupRef, ContentProps>((props, ref) => {
  ...
  function cleanSelectIds() {
    for (let index = 0; index < zhButton.length; index += 1) {
      refs[index]?.onReset();
    }
  }

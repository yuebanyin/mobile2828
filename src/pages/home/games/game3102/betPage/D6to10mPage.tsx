...
import {
 ButtonDataType, ContentProps, ItemLayoutProps, d6to10Button 
} from '../rules';
...
const ItemLayout = forwardRef<RadioGroupRef, ItemLayoutProps>((props, ref) => {
 ...
 data, gameMultiple, dyMultiple, onChange 
  ...
  const onItemClass = useCallback((index: number) => `${styles['h-item-box']} ${styles[`w-pr-${index < 4 ? '4' : '5'}`]}`, []);
 ...
 * 幸运飞艇-6~10名
 */
export const D6to10mPage = forwardRef<RadioGroupRef, ContentProps>((props, ref) => {
  ...
  function cleanSelectIds() {
    for (let index = 0; index < d6to10Button.length; index += 1) {
      refs[index]?.onReset();
    }
  }

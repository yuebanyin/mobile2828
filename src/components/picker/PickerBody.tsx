import React, {
 useState, useEffect, useRef, ForwardRefRenderFunction, useImperativeHandle, forwardRef, useCallback 
} from 'react';
...
const InternalPickerSlot: ForwardRefRenderFunction<{ stopMomentum: () => void; moving: boolean }, Partial<PickerBodyProps>> = (props, ref) => {
  const {
 keyIndex = 0, defaultValue, listData = [], threeDimensional = true, swipeDuration = 1000, chooseItem 
} = props;
  ...
  const [touchHeight, setTouchHeight] = useState(-36);
  const rotation = 30;
  ...
  const setTransform = (translateY = 0, type: string, time = DEFAULT_DURATION, deg: string) => {
    let nTime = time;
    if (type !== 'end') {
      nTime = 0;
    }
    ...
    setTouchHeight(translateY); // 设置偏移量
  };
  ...
  // 阻止默认事件
  const preventDefault = (event: React.TouchEvent<HTMLElement>, isStopPropagation?: boolean) => {
  ...
  const modifyStatus = useCallback(
    ...
    [defaultValue, listData, setChooseValue, setMove]
  );
  ...
  const touchRollerStyle = () => ({
    ...
    transform: `translateY(${touchHeight}px)`,
  ...
  return (
    <div className='bx-picker-list' ref={pickerSlotRef} onTouchStart={touchStart} onTouchMove={touchMove} onTouchEnd={touchEnd}>
      <div className='bx-picker-roller nut-picker-roller' ref={rollerRef} style={threeDimensional ? touchRollerStyle() : touchTileStyle()} onTransitionEnd={stopMomentum}>
        {/* 3D 效果 */}
        {threeDimensional
          && listData.map((item, index) => (
            <div
              className={`bx-picker-roller-item nut-picker-roller-item b-b-1 bc-split ${lineSpacing * index === -touchHeight ? 'color-primary' : ''} ${
                isHidden(index + 1) && 'bx-picker-roller-item-hidden nut-picker-roller-item-hidden'
              }`}
              // style={{
              //   transform: `rotate3d(1, 0, 0, ${-rotation * (index + 1)}deg) translate3d(0px, 0px, 104px)`,
              // }}
              style={{
                transform: `translateY(${lineSpacing * index}px) `,
              }}
      ...
      <div className='bx-picker-mask nut-picker-mask' />
      <div className='bx-picker-indicator font-w-bolder nut-picker-indicator b-t-1 b-b-1 bc-split' />
    </div>
  );
};
const PickerSlot = forwardRef(InternalPickerSlot);
